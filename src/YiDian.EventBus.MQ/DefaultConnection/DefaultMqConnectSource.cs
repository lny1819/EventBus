...
namespace YiDian.EventBus.MQ.DefaultConnection
{
    public class DefaultMqConnectSource
    {
        ...
        public DefaultMqConnectSource(IAppEventsManager eventsManager, IEventSeralize seralize, ILogger<IEventBusSubManager> sub_logger, int retryCount, IEventBusSubManagerFactory factory)
        {
        ...
        public DefaultRabbitMQPersistentConnection Create(string mqConnstr)
        {
            ...
            var mqconn = new DefaultRabbitMQPersistentConnection(conn, source_name, _retryCount);
