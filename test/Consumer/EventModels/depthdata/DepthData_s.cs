...
namespace EventModels.depthdata
{
    public partial class DepthData : IYiDianSeralize
    {
        ...
        public void BytesTo(ref ReadStream stream)
        {
            ...
            if (headers.TryGetValue(EventPropertyType.L_Date, out count))
            {
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (index == 62) { TradingDay = stream.ReadDate(); continue; }
                    stream.Advance(11);
                }
            }
            ...
            if (headers.TryGetValue(EventPropertyType.L_64, out count))
            {
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (index == 39) { PreOpenInterest = stream.ReadUInt64(); continue; }
            ...
            if (headers.TryGetValue(EventPropertyType.L_Str, out count))
            {
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (index == 63) { TradingTime = stream.ReadString(); continue; }
        ...
        {
            var size = 542 + WriteStream.GetStringSize(CurrencyNo, encoding) + WriteStream.GetStringSize(CommodityNo) + WriteStream.GetStringSize(ExchangeID) + WriteStream.GetStringSize(InstrumentID) + WriteStream.GetStringSize(Source) + WriteStream.GetStringSize(TradingTime) + 0;
            return size;
        }
    }
}
